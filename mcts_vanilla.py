...
def expand_leaf(node, board, state):
    ...
    new_node = MCTSNode(parent=node, parent_action=action_update, action_list=board.legal_actions(board.next_state(state,action_update)))
...
def think(board, state):
    ...
    for keys in node.child_nodes.keys():      #Error here
        #print("child ", children)
        if node.child_nodes[keys].visits > max_child_visits:
            max_child_visits = node.child_nodes[keys].visits
            selected_action = node.child_nodes[keys].parent_action
